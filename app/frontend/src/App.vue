...
async function get_image_from_video(video_name, timestamp) {
    try {
        const response = await fetch(`/image-from-video/${video_name}?timestamp=${timestamp}`);
        if (!response.ok) {
...
async function search() {
    ...
    try {
        const response = await fetch(`/search-embeddings?text=${encodeURIComponent(search_query.value)}&top_k=${search_top_k.value}`);
...
<template>
    <div class="container">
        <header class="page-header">
            <h1>Embedding-based search in traffic videos</h1>
        </header>
