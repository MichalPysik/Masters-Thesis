...
# Function to sample frames from the video and store embeddings in DB
...
def get_image_text_similarity(image_path: str, text: str) -> float:
    """
    Computes the cosine similarity between an image and a text description using chosen Multimodal embedding model.
    ...
    with torch.no_grad():
        image_features = emb_model.get_image_features(inputs["pixel_values"])
        text_features = emb_model.get_text_features(
